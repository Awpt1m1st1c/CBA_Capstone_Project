...
const AIPropertyHub = () => {
  ...
  const handleSearch = async (searchParams) => {
    ...
    try {
      ...
      // Ensure we handle the response correctly even if locations array is empty
      if (locationResponse && locationResponse.success !== false) {
        const locationsData = locationResponse.locations || [];
        const analysisData = locationResponse.analysis || "";
        
        console.log("Setting locations:", locationsData.length, "items");
        console.log("Setting analysis:", analysisData ? "Has content" : "Empty");
        
        setLocations(locationsData);
        // Clean up the analysis text (remove extra newlines, ensure proper formatting)
        const cleanedAnalysis = analysisData.trim().replace(/\n\n\n+/g, '\n\n');
        setLocationAnalysis(cleanedAnalysis);
      } else {
        // If response indicates failure, still set empty arrays but show analysis if available
        setLocations([]);
        const cleanedAnalysis = (locationResponse?.analysis || "").trim().replace(/\n\n\n+/g, '\n\n');
        setLocationAnalysis(cleanedAnalysis);
      }
    } catch (error) {
      console.error("Error during search:", error);
      // Don't set error if we got partial data - just log it
      if (error.response?.status >= 500) {
        setSearchError("Failed to fetch some data. Partial results may be available.");
      } else {
        setSearchError(error.response?.data?.message || "Failed to fetch property data. Please try again.");
      }
      ...
      // Ensure searchPerformed is set to true after search completes
      setSearchPerformed(true);
      
      // Force a small delay to ensure state updates propagate
      setTimeout(() => {
        console.log("Search completed. Loading:", false, "Search performed: true");
      }, 100);
    }
  };
          ...
          {!isLoading && searchPerformed && (
            <div className="space-y-8 sm:space-y-12">
              ...
              <div className="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 md:p-8">
                ...
                {/* Show analysis even if locations array is empty */}
                {locationAnalysis && locationAnalysis.trim() && (
                  <div className="mt-6 sm:mt-8">
                    <AnalysisDisplay analysis={locationAnalysis} />
                  </div>
                )}
                
                {/* Show message if we have analysis but no location data */}
                {locationAnalysis && locationAnalysis.trim() && (!locations || locations.length === 0) && (
                  <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-100">
                    <p className="text-sm text-blue-700">
                      <strong>Note:</strong> Location-specific data is currently unavailable, but we've provided general market insights based on available information.
                    </p>
                  </div>
                )}
              </div>
